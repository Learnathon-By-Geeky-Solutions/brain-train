...
const Analysis = ({ show, imagePreview, analysisResult, resetComponent }) => {
  ...
  const buttonName =
    type === "nutrition"
      ? "View Similar Food Recipes"
      : "View Recipes with Detected Ingredients";
      ...
      <Flex direction={{ base: "column", md: "row" }} gap={6} mb={6}>
        ...
        <Box flex="2">
          <Heading size="2xl" mb={2}>
            {type === "nutrition"
              ? initCap(analysisResult.category.name)
          ...
          {type === "nutrition" && (
            <Flex align="center" my={3}>
              <Box
                borderRadius="full"
                bg={
                  analysisResult.category.probability * 100 > 50
                    ? "green.100"
                    : "red.100"
                }
                color="green.700"
              ...
              >
                {analysisResult.category.probability.toFixed(4) * 100}%
                Probability
              </Text>
            </Flex>
          )}

          <Button colorScheme="blue" mt={4} size="sm" onClick={resetComponent}>
        ...
        <div>
          ...
          <Box mt={6} p={4} bg="blue.50" borderRadius="md">
            ...
            <Text fontSize="sm" color="blue.700">
              Ingredients are detected using computer vision and may not capture
              all components, especially spices and smaller ingredients. Higher
              confidence percentages indicate more reliable detection.
            </Text>
          </Box>
        </div>
      )}
...
Analysis.propTypes = {
  ...
  imagePreview: PropType.string.isRequired,
  analysisResult: PropType.shape({
    type: PropType.string.isRequired,
    category: PropType.shape({
      name: PropType.string.isRequired,
      probability: PropType.number.isRequired,
    }),
    ingredients: PropType.arrayOf(
      PropType.shape({
        name: PropType.string.isRequired,
        confidence: PropType.number.isRequired,
      }),
    ),
    nutrition: PropType.shape({
      calories: PropType.object.isRequired,
      protein: PropType.object.isRequired,
      carbs: PropType.object.isRequired,
      fat: PropType.object.isRequired,
    }),
