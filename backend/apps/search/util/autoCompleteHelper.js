...
export const getDbSuggestions = (query, number) =>
    getRecipeFieldsByTitle(query, ['_id', 'title'], number, true)
      .then((dbResults) => {
...
export const getApiSuggestions = (query, number, existingSuggestions) => {
    const seenTitles = new Set(existingSuggestions.map((s) => s.title.toLowerCase()));
  
    return spoonacularRequest('/recipes/autocomplete', { query, number })
      .then((apiResults) => {
