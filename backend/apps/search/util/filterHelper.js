export const normalizeFilters = (filters) => ({
  ...
  cuisineList: (filters.cuisine || "")
    .split(",")
    .map((c) => c.trim().toLowerCase())
    .filter(Boolean),
  dietList: (filters.diet || "")
    .split(",")
...
export const extractNutrients = (nutrients = []) => {
  const find = (name) =>
    nutrients.find((n) => n.name?.toLowerCase() === name)?.amount || 0;
  return {
...
export const meetsListCriteria = (dataList = [], expectedList = []) =>
  ...
  (dataList || []).some((d) => expectedList.includes(d.toLowerCase()));

export const meetsCuisineCriteria = (cuisines = [], expected = []) =>
  expected.length === 0 ||
  cuisines.some((c) => expected.includes(c.toLowerCase()));

export const meetsBooleanCriteria = (actual, expected) =>
  Object.entries(expected).every(
    ([key, val]) => val === null || actual[key] === val,
  );
