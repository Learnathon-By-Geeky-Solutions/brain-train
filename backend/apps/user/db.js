...
export const getSimilarRecipesById = async (recipeId) => {
  ...
  if (mongoose.Types.ObjectId.isValid(_recipeId))
    return await SimilarRecipe.findOne({ recipeId: { $eq: _recipeId } }, null, {
      sanitizeFilter: true,
    });
...
export const createNewSimilarRecipeEntry = async (
  recipeId,
  similarRecipeIds,
) => {
  try {
