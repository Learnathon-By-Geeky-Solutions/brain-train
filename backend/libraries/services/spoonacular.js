...
const API_KEY =
  ...
  (() => {
    console.error(
      "[ERROR] SPOONACULAR_API_KEY IS NOT DEFINED IN ENVIRONMENT VARIABLES",
    );
...
export const spoonacularRequest = async (endpoint, params = {}) => {
  ...
  try {
    const response = await axios.get(
      `${SPOONACULAR_API_BASE_URL}${formattedEndpoint}`,
      {
    ...
    console.error(
      `[Spoonacular API Error] ${error.message} | ENDPOINT: ${formattedEndpoint}`,
    );
    console.error(`[Error Details]`, error.response?.data || {});
    throw new Error(
      error.response?.data?.message || "Spoonacular API request failed",
    );
  }
};
